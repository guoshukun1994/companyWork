...
var chain3 = new Chain3(new Chain3.providers.HttpProvider("http://testnode.moacchina.info")); //http://node.moacchina.info
...
//在区块链中查询货物状态
async function callContract(code, address = "") {
    ...
    if (data == 0) {
        ...
        console.log("验证码");
        //  await goodsTagInfo.update({ count: count + 1 }, { where: { check_code: code } });
        try {
            //查询Tag表中的商品信息
            ...
            //查询公司表中对应公司的信息
            let compInfoMessage = (await compInfo.findOne({
            ...
            //   console.log(compInfoMessage);
            console.log("1");
            return { ...goodsTagMessage, ...goodsInfoMessage, ...compInfoMessage };
        } catch (e) {
            console.log(e);
            return "数据获取异常";
        }
        ...
        var data = Contract.useGoods.getData(code, "666555");
        var txCount = global.nonce - 1;
        var rawTx = {
            nonce: chain3.intToHex(txCount),
            gasPrice: chain3.intToHex(30000000000),
            gasLimit: chain3.intToHex(500000),
            to: contractAddress,
            data: data,
                                ...
                                if (receipt.status != "0x0") {
                                    ...
                                    if (address != "") {
                                            ...
                                            .update(
                                                {
                                                    ...
                                                    detailedAddress: add.detailedAddress,
                                                },
                                                {
                                                    where: {
                                                        consume_code: code,
                                        ...
                                        goodsTagInfo //修改Tag表中商品的状态
                                            ...
                                            .then(result => {
                                                console.log("else Goods update success");
                                            })
                                            .catch(err => {
                                                console.log("else Goods update fail", err);
                                            });
                                    }
                                } else {
                                    reject("消费失败");
            ...
            try {
                ...
                eventDetails.watch(async function(e, result) {
                    if (!e) {
                        ...
                        // console.log("details事件", result);
                        // console.log("str:", str);
                        //获取与传入的OpenID一致的事件返回值
                        if (result.args.openID == "666555" && result.args.cCode == str) {
