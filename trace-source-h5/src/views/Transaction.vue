<template>
  <div class="transaction">
    ...
    <div class="traDetail">
      ...
      <div class="traContent">
        <div>燃料上限</div>
        <div>{{ traDetail.gas }}</div>
      </div>
      <div class="traContent">
        <div>燃料价格</div>
        <div>{{ traDetail.gasPrice }} MOAC</div>
      </div>
      <div class="traContent">
        <div>燃料消耗</div>
        <div>{{ traDetail.gasUsed }}</div>
      </div>
      <div class="traContent">
        <div>交易费用</div>
        <div>{{ traDetail.txCost }} MOAC</div>
      </div>
      <div class="traContent">
        <div>操作数</div>
        <div>{{ traDetail.nonce }}</div>
      </div>
      <div class="traContent">
        <div>交易状态</div>
        <div>{{ traDetail.status === "0x1" ? "成功" : "失败" }}</div>
      </div>
      <div class="traContent">
        <div>附加数据</div>
...
export default {
  ...
  created() {
    ...
    getTxHashMsg({ txHash: hash }).then((res) => {
      console.log(res);
